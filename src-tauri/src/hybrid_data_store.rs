    ...
    if HybridDataStore::should_use_sqlite(&app_handle) {
        ...
        match SqliteDataStore::new(&app_handle) {
            ...
            Err(e) => {
                ...
                crate::data_store::save_chat_sessions(app_handle, payload)
            }
        }
    } else {
        // Use JSON (legacy)
...
pub fn load_chat_sessions_hybrid(app_handle: AppHandle) -> Result<LoadChatsResponse, String> {
    if HybridDataStore::should_use_sqlite(&app_handle) {
        ...
        match SqliteDataStore::new(&app_handle) {
            ...
            Err(e) => {
                println!("⚠️ SQLite failed, falling back to JSON: {}", e);
                crate::data_store::load_chat_sessions(app_handle)
            }
        }
    } else {
        // Use JSON (legacy)
...
pub fn save_conversations_hybrid(
    ...
    if HybridDataStore::should_use_sqlite(&app_handle) {
        ...
        match SqliteDataStore::new(&app_handle) {
            ...
            Err(e) => {
                println!("⚠️ SQLite failed, falling back to JSON: {}", e);
                crate::data_store::save_conversations(app_handle, payload)
            }
        }
    } else {
        // Use JSON (legacy)
...
pub fn load_conversations_hybrid(app_handle: AppHandle) -> Result<LoadConversationsResponse, String> {
    if HybridDataStore::should_use_sqlite(&app_handle) {
        ...
        match SqliteDataStore::new(&app_handle) {
            ...
            Err(e) => {
                println!("⚠️ SQLite failed, falling back to JSON: {}", e);
                crate::data_store::load_conversations(app_handle)
            }
        }
    } else {
        // Use JSON (legacy)
...
pub fn delete_conversation_hybrid(
    ...
    crate::data_store::delete_conversation(app_handle, conversation_id)
}
...
pub fn restore_from_backup_hybrid(
    ...
    crate::data_store::restore_from_backup(app_handle, backup_type, backup_filename)
}
...
pub fn save_conversation_message_hybrid(
    ...
    crate::data_store::save_conversation_message(app_handle, session_id, message)
}
...
pub fn batch_save_conversation_messages_hybrid(
    ...
    crate::data_store::batch_save_conversation_messages(app_handle, session_id, messages)
}
...
pub fn save_conversation_insight_hybrid(
    ...
    crate::data_store::save_conversation_insight(app_handle, session_id, insight)
}
...
pub fn get_conversation_insights_hybrid(
    ...
    crate::data_store::get_conversation_insights(app_handle, session_id)
}